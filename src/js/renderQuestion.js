...
function clearChoicesLayoutClasses() {
  if (!quizBox?.choices) return;
  quizBox.choices.classList.remove("choices--cards", "choices--text", "choices--scale");
}

function renderScaleQuestion(q) {
  clearChoicesLayoutClasses();
  quizBox.choices.classList.add("choices--scale");
  ...
  if (hasChoices) {
    const selected = answers[q.id] ?? q.default ?? "";

    const choicesRow = document.createElement("div");
    choicesRow.className = "choices choices--cards";

    q.choices.forEach((c, i) => {
      const id = `${q.id}_${i}`;

      const row = document.createElement("div");
      row.className = "choiceRow choiceRow--card";
      ...
      input.id = id;
      input.value = c.value;
      input.checked = String(c.value) === String(selected);
      ...
      label.className = "choiceLabel choiceLabel--card";

      const choicePicto = getChoicePictoFile(c);
      if (choicePicto) {
        const img = document.createElement("img");
        img.className = "choicePicto choicePicto--card";
      ...
      span.className = "choiceText choiceText--card";
  ...
  facesRow.innerHTML = faces
    .map((f, i) => {
      const txt = escapeHtml(labels[i] || "");
      return `
        <div class="scaleFaceWrap">
          <div class="scaleFace" aria-hidden="true">${f}</div>
          <div class="scaleFaceLabel">${txt}</div>
        </div>
      `;
    })
    .join("");
  ...
  const valueOut = document.createElement("div");
  valueOut.className = "scaleValue";
...
function renderQuestion() {
  ...
  if (type === "text") {
    quizBox.choices.classList.add("choices--text");
    ...
    quizBox.choices.classList.add("choices--cards");
